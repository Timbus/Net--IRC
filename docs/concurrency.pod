...
Small pieces of work, in a partially ordered list.

=item1 All non-event-local mutation via API, not direct access
=item1 Abstract IRC, console, and log connections behind channels
=item1 Each event in a separate task
=item1 Move all connections out to non-core modules
=item1 Implement simple K/V store
=item1 Implement simple HTTP GET
=item1 Clean up APIs, making sure both sync and async forms exist
=item1 Reassess current design and progress; add more inch pebbles
